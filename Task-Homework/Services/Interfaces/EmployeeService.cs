...
namespace Task_Homework.Services.Interfaces
{
    public class EmployeeService : IEmployeeService
    {
        ...
        public EmployeeResponse GetSearchUser(Employee[] employees,string name,string surname) 
        {
            if(name==null)
            {
            ...
            Employee employee=employees.FirstOrDefault(m => m.Name==name||m.Surname==surname);

            if(employee is null) 
            {
                return new EmployeeResponse { ErrorMessage = "Name Username not found" };
            }
            return new EmployeeResponse { Employee = employee };
